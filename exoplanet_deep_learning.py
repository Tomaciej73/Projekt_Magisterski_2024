...
def perform_eda(df):
    ...
    sns.histplot(df['Mass'], bins=30, kde=True, color='skyblue')
    ...
    plt.figure(figsize=(10, 6))
    sns.scatterplot(data=df, x='Orbital Period Days', y='Mass', hue='Discovery Method', palette='viridis', alpha=0.6)
...
def analyze_missing_values(df):
    ...
    # Usunięcie kolumn z bardzo wysokim procentem braków, np. powyżej 50%
    cols_to_drop = missing_df[missing_df['Missing Percentage'] > 50]['Column']
    df = df.drop(cols_to_drop, axis=1)
    print(f"\nUsunięto kolumny z >50% braków: {list(cols_to_drop)}")
...
def remove_outliers(df, threshold=3):
    ...
    print(f"\nUsunięto {df.shape[0] - df_clean.shape[0]} wartości odstających.")
    return df_clean
...
def main():
    ...
    # Użyj OneHotEncoder dla multi-class
    ohe = OneHotEncoder(sparse_output=False)  # Poprawiony argument
...
- Po usunięciu kolumn z >50% braków: {df.shape[0]}
