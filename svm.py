...
# 16. Ważność cech (po PCA) - SVM nie oferuje bezpośredniej metody ważności cech, więc pomijamy ten krok.
...
print(predictions_df_svm.to_markdown())
...
# 21. Predykcja na podstawie przykładowych danych dla SVM
predictions_svm = svm.predict(mock_pca)
mockDF["Pulsar Star Predictions - SVM"] = predictions_svm

print("\nPrzykładowe predykcje dla nowych danych - SVM:")
print(mockDF.to_markdown())

# 22. Podsumowanie wyników
true_positives_svm = np.sum((y_test == 1) & (y_pred == 1))
false_positives_svm = np.sum((y_test == 0) & (y_pred == 1))
true_negatives_svm = np.sum((y_test == 0) & (y_pred == 0))
false_negatives_svm = np.sum((y_test == 1) & (y_pred == 0))

summary_df_svm = pd.DataFrame({
    "Nazwa": ["Prawdziwie pozytywne", "Fałszywie pozytywne", "Prawdziwie negatywne", "Fałszywie negatywne"],
    "Zliczone wartości": [true_positives_svm, false_positives_svm, true_negatives_svm, false_negatives_svm]
})

print("\nPodsumowanie wyników - SVM:")
print(summary_df_svm.to_markdown())
